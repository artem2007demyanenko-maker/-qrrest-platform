...
test.describe('Loyalty operational smoke checks', () => {
  test('staff loyalty lookup page renders safely', async ({ page, diagnostics }) => {
    ...
    skipOrFail(test.skip, !credentials, 'Set staff/waiter/owner credentials to run loyalty staff smoke checks.');
  ...
  test('restaurant loyalty settings page renders safely', async ({ page, diagnostics }) => {
    skipOrFail(test.skip, !ownerCredentials, 'Set QRREST_OWNER_EMAIL/PASSWORD or QRREST_ADMIN_EMAIL/PASSWORD to run loyalty owner smoke checks.');
