...
test.describe('Staff operational flow', () => {
  test('staff or waiter can open orders panel', async ({ page, diagnostics }) => {
    ...
    skipOrFail(test.skip, !credentials, 'Set QRREST_WAITER_EMAIL/PASSWORD or QRREST_STAFF_EMAIL/PASSWORD to run staff smoke checks.');
  ...
  test('staff or waiter can open floorplan panel', async ({ page, diagnostics }) => {
    const credentials = waiterCredentials || staffCredentials;
    skipOrFail(test.skip, !credentials, 'Set QRREST_WAITER_EMAIL/PASSWORD or QRREST_STAFF_EMAIL/PASSWORD to run floorplan smoke checks.');
