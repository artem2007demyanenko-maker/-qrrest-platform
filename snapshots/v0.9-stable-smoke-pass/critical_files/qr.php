...
// Убрать из сессии позиции, которых больше нет в меню или которые скрыты (available=0) — иначе «хвосты» после правок в owner panel.
if (!is_demo_mode() && $cart && isset($currentRestaurant['id'])) {
    ...
    if ($rawIds) {
        ...
        $chk = $pdo->prepare(
            "SELECT id FROM menu_items WHERE restaurant_id = ? AND available = 1 AND id IN ($placeholders)"
        );
