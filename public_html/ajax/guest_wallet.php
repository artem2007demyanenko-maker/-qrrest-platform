...
try {
    if (!function_exists('guest_history_resolve_phone') || !function_exists('loyalty_guest_balance') || !function_exists('loyalty_guest_ledger')) {
        throw new RuntimeException('wallet_helpers_missing');
    }
    ...
    $ledger = loyalty_guest_ledger($pdo, $restaurantId, $resolved, $limit);
    $canSpendDemo = function_exists('loyalty_wallet_can_spend')
        ? loyalty_wallet_can_spend($wallet, 1)
        : ((int)($wallet['current_balance'] ?? 0) > 0);
    ...
    echo json_encode([
        ...
        'guest' => [
            ...
            'guest_id' => (int)($wallet['guest_id'] ?? 0),
            'phone_normalized' => (string)($resolved['phone_normalized'] ?? ''),
            'guest_name' => (string)($wallet['guest_name'] ?? ($resolved['guest_name'] ?? 'Гость')),
        ...
        'wallet' => [
            'source' => (string)($wallet['source'] ?? 'none'),
            'current_balance' => (int)($wallet['current_balance'] ?? 0),
            'total_earned' => (int)($wallet['total_earned'] ?? 0),
            'total_spent' => (int)($wallet['total_spent'] ?? 0),
            'total_refund' => (int)($wallet['total_refund'] ?? 0),
            'total_adjusted' => (int)($wallet['total_adjusted'] ?? 0),
            'can_spend_min_1' => (bool)$canSpendDemo,
            'card_id' => (int)($wallet['card_id'] ?? 0),
        ],
        'ledger' => $ledger,
    ], JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
    error_log('GUEST_WALLET_ENDPOINT_FAIL restaurant_id=' . $restaurantId . ' ' . $e->getMessage());
