...
if ($orders) {
    ...
    if ($orderIds !== []) {
        ...
        $hasMenuItemProdStationCol = function_exists('db_column_exists') && db_column_exists('menu_items', 'production_station');
        $hasMenuItemStationCol = function_exists('db_column_exists') && db_column_exists('menu_items', 'station');
        $hasMenuItemKitchenStationCol = function_exists('db_column_exists') && db_column_exists('menu_items', 'kitchen_station');
        $stationExpr = "LOWER(COALESCE("
            . "NULLIF(TRIM(" . ($hasOrderItemProdStationCol ? "oi.production_station" : "''") . "), ''),"
            . "NULLIF(TRIM(" . ($hasMenuItemProdStationCol ? "mi.production_station" : "''") . "), ''),"
            . "NULLIF(TRIM(" . ($hasMenuItemStationCol ? "mi.station" : "''") . "), ''),"
            . "NULLIF(TRIM(" . ($hasMenuItemKitchenStationCol ? "mi.kitchen_station" : "''") . "), ''),"
            . "'kitchen')) AS item_station_key";
        $stmt = $pdo->prepare("
            SELECT oi.*, mi.name AS menu_name, {$stationExpr}
            FROM order_items oi
    ...
    $itemsByOrder = [];
    foreach ($rows as $row) {
        $stationKeyRaw = function_exists('kds_normalize_station')
            ? kds_normalize_station((string)($row['item_station_key'] ?? 'kitchen'))
            : strtolower(trim((string)($row['item_station_key'] ?? 'kitchen')));
        $stationLabel = function_exists('kds_station_label_ru')
            ? kds_station_label_ru($stationKeyRaw)
            : $stationKeyRaw;
        $itemsByOrder[$row['order_id']][] = [
            'menu_name' => (string)($row['menu_name'] ?? ''),
            'quantity'  => (int)($row['quantity'] ?? $row['qty'] ?? 1),
            'price'     => (float)($row['price'] ?? 0),
            'station'   => $stationKeyRaw,
            'station_label' => $stationLabel,
        ];
    }

    $progressByOrder = [];
    $hasStatusCol = (function_exists('db_column_exists') && db_column_exists('order_items', 'kds_status'))
        || (function_exists('db_column_exists') && db_column_exists('order_items', 'station_status'));
    if ($hasStatusCol) {
        $itemStatusExpr = function_exists('kds_item_status_sql_expr')
